...
function StatusBadge({ status }: { status: StatusUsuario }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${cfg.dot} ${status === 'Ativo' ? 'animate-pulse' : ''}`}
      />
      {status}
    </span>
  )
...
const gruposPermissao: GrupoPermissao[] = [
  {
    ...
    itens: [
      {
        key: 'visualizarProcessos',
        label: 'Visualizar processos',
        desc: 'Acesso de leitura a todos os processos e casos do escritório.'
      },
      {
        key: 'criarProcessos',
        label: 'Criar novos processos',
        desc: 'Permissão para abrir e cadastrar novos processos no sistema.'
      },
      {
        key: 'editarProcessos',
        label: 'Editar processos',
        desc: 'Modificar dados, partes, prazos e movimentações de processos.'
      },
      {
        key: 'excluirProcessos',
        label: 'Excluir processos',
        desc: 'Remoção permanente de processos. Ação irreversível.',
        danger: true,
        destaque: true
      }
    ]
  ...
  },
  {
    ...
    itens: [
      {
        key: 'editarPerfisSite',
        label: 'Editar perfis do site',
        desc: 'Atualizar fotos, bios e especialidades dos advogados no portal.',
        destaque: true
      },
      {
        key: 'publicarConteudo',
        label: 'Publicar conteúdo',
        desc: 'Criar e publicar artigos, notícias e atualizações institucionais.'
      }
    ]
  },
  {
    ...
    itens: [
      {
        ...
        desc: 'Baixar relatórios em CSV/PDF com dados de processos e clientes.'
      },
      {
        key: 'acessarPainelAdmin',
        label: 'Acessar painel admin',
        desc: 'Visualizar métricas, logs e configurações avançadas do sistema.',
        danger: true
      },
      {
        key: 'gerenciarUsuarios',
        label: 'Gerenciar usuários',
        desc: 'Adicionar, editar e remover membros da equipe e seus acessos.',
        danger: true
      }
    ]
  }
        ...
        <div className="mx-6 mt-5 flex items-start gap-2.5 p-3.5 rounded-xl bg-amber-50 border border-amber-200">
          ...
          <div>
            <p className="text-xs font-semibold text-amber-700">
              Perfil Estagiário — Acesso Restrito
            </p>
            <p className="text-xs text-amber-600 mt-0.5">
              Permissões críticas estão desativadas por padrão para este nível. Ative
              individualmente com cautela.
            </p>
          </div>
        </div>
      )}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          {isDng && (
                            <Lock
                              className={`w-3 h-3 flex-shrink-0 ${isOn ? 'text-red-500' : 'text-slate-300'}`}
                            />
                          )}
                          <p
                            className={`text-sm font-medium ${isDng && isOn ? 'text-red-700' : 'text-[#1A2B3C]'}`}
                          >
                            {item.label}
                          </p>
                          {isDng && (
...
export function TeamManagement() {
  ...
  const [permModal, setPermModal] = useState<{ isOpen: boolean; usuario: Usuario | null }>({
    isOpen: false,
    usuario: null
  })
  ...
  const removerUsuario = async (id: string) => {
    ...
    try {
      ...
      setErroDelete('Erro ao remover usuário. Tente novamente.')
    } finally {
      setDeletando(false)
    }
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="text-sm text-slate-600 mb-4">
              ...
              <strong>{usuarios.find(u => u.id === confirmDelete)?.nome}</strong> do sistema? O
              usuário será excluído do banco de dados e do Keycloak.
            </p>
            {erroDelete && (
              <div className="flex items-center gap-2 p-3 mb-4 rounded-lg bg-red-50 border border-red-100">
                <AlertTriangle className="w-4 h-4 text-red-500 flex-shrink-0" />
                <p className="text-xs text-red-600">{erroDelete}</p>
              </div>
            )}
            <div className="flex gap-3">
              ...
              <button
                onClick={() => removerUsuario(confirmDelete)}
                disabled={deletando}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-gray-100">
                <tr>
                  {['Nome', 'E-mail', 'Nível de Acesso', 'Status', 'Permissões', 'Ações'].map(
                    col => (
                      <th
                        key={col}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {(['Admin', 'Advogado', 'Estagiário'] as NivelAcesso[]).map(nivel => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${nivel === 'Admin' ? 'bg-[#1A2B3C]' : nivel === 'Advogado' ? 'bg-blue-100' : 'bg-amber-100'}`}
                  >
                    <Icon
                      className={`w-4 h-4 ${nivel === 'Admin' ? 'text-white' : nivel === 'Advogado' ? 'text-blue-600' : 'text-amber-600'}`}
                    />
                  </div>
                  <div>
