...
export function NovoClienteModal({ isOpen, onClose, onClienteCriado }: NovoClienteModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.detail || 'Ocorreu um erro ao criar o cliente. Verifique os dados.'
      )
    } finally {
      setLoading(false)
    }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      {/* Backdrop */}
      <div
        className="absolute inset-0 bg-[#1A2B3C]/40 backdrop-blur-[2px] pointer-events-auto"
        onClick={onClose}
      />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/50 text-xs mt-0.5">
                Cadastrar cliente para vinculação de processos
              </p>
            </div>
          </div>
          <button
              ...
              <label className="flex items-start gap-3 p-4 rounded-xl border border-[#D4AF37]/30 bg-[#D4AF37]/5 cursor-pointer hover:bg-[#D4AF37]/10 transition-colors group">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                    Declaro que o cliente forneceu autorização expressa para o monitoramento ativo e
                    contínuo de seus dados sensíveis via DataJud.
                  </p>
                </div>
              </label>
            </div>
