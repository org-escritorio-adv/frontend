...
function highlightMatch(text: string, query: string) {
  ...
  return (
    <span>
      {text.slice(0, idx)}
      <mark className="bg-[#D4AF37]/30 text-[#1A2B3C] rounded px-0.5 not-italic">
...
export function PrazosCalculadoraModal({ isOpen, onClose }: PrazosCalculadoraModalProps) {
  ...
  const recalcular = useCallback(async () => {
    ...
    try {
      ...
      if (apenasUteis && diasEfetivos > 0) {
        ...
        resLocal.diasCorridos = Math.round(
          (dataFinalBackend.getTime() - base.getTime()) / 86_400_000
        )
      }
  ...
  const gerarNota = () => {
    ...
    const partes: string[] = []
    if (fdsSemana > 0)
      partes.push(`${fdsSemana} ${fdsSemana === 1 ? 'final de semana' : 'finais de semana'}`)
  ...
  const corUrgencia = resultado?.critico
    ? {
        ...
        icon: <CalendarX className="w-4 h-4 text-red-500" />
      }
    : resultado?.urgente
      ? {
      ...
      : {
          ring: 'border-[#1A2B3C]/20 bg-[#1A2B3C]/5',
          text: 'text-[#1A2B3C]',
          badge: 'bg-[#1A2B3C]/10 text-[#1A2B3C]',
          icon: <CalendarCheck className="w-4 h-4 text-[#1A2B3C]" />
        }
      ...
      >
        ...
        <div className="px-6 pt-5 pb-4 space-y-5 overflow-y-auto flex-1">
          ...
          <div ref={autocompleteRef} className="relative">
              ...
              <div className="flex items-center gap-3 px-4 py-3 border-2 border-[#1A2B3C]/30 bg-[#1A2B3C]/5 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 truncate">
                    {processoSelecionado.partes || 'Sem partes'} · {processoSelecionado.tribunal}
                  </p>
                </div>
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 bg-slate-100 text-slate-600 uppercase tracking-wider`}
                >
            ...
            {!processoSelecionado && (
              <p className="text-[10px] text-slate-400 mt-1.5 flex items-center gap-1">
                Selecione um processo para poder <strong className="text-slate-500">Salvar</strong>{' '}
                a data no sistema.
              </p>
            )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-500 mt-0.5 truncate">
                          {p.partes ? highlightMatch(p.partes, processoBusca) : 'Sem partes'} ·{' '}
                          {p.tribunal}
                        </p>
                      </div>
                    </button>
            ...
            resultado && (
              <div className={`rounded-2xl border-2 p-5 transition-all ${corUrgencia.ring}`}>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span
                    className={`flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full ${corUrgencia.badge}`}
                  >
                ...
                <div className="mb-3">
                  <p
                    className={`text-[28px] font-bold leading-none tracking-tight ${corUrgencia.text}`}
                  >
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    ...
                    {apenasUteis &&
                      resultado.diasUteisContados > 0 &&
                      ` · ${resultado.diasUteisContados} dias úteis`}
                  </p>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {resultado.fdsSemana > 0 && (
                      <span className="flex items-center gap-1 text-[11px] px-2.5 py-1 bg-white/80 rounded-lg border border-current/10 text-slate-500">
                        📅 {resultado.fdsSemana}{' '}
                        {resultado.fdsSemana === 1 ? 'fim de semana' : 'fins de semana'}
                      </span>
                    )}
                ...
                {(resultado.urgente || resultado.critico) && (
                  <div
                    className={`flex items-center gap-2 mt-3 p-2.5 rounded-lg ${resultado.critico ? 'bg-red-100' : 'bg-amber-100'}`}
                  >
                    <AlertTriangle
                      className={`w-3.5 h-3.5 flex-shrink-0 ${resultado.critico ? 'text-red-500' : 'text-amber-500'}`}
                    />
                    <p
                      className={`text-xs font-semibold ${resultado.critico ? 'text-red-700' : 'text-amber-700'}`}
                    >
                      {resultado.critico
          ...
          <div className="flex items-start gap-2 text-[11px] text-slate-400 pb-1">
            ...
            <p>
              Este cálculo é uma estimativa com base nas configurações selecionadas. Sempre
              verifique os feriados forenses do TJDFT e eventuais suspensões de prazo antes de
              protocolar.
            </p>
          </div>
        </div>
