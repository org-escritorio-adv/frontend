...
import {
  UserPlus, Mail, Phone, Calendar, Trash2, Edit,
  MessageSquare, Briefcase, X, CheckCircle2, Loader2,
  Shield, Users, Settings2, AlertTriangle,
} from "lucide-react";
...
const nivelConfig: Record<NivelAcesso, { badge: string; dot: string; icon: typeof Shield }> = {
  ...
  Advogado:   { badge: 'bg-blue-50 text-blue-700 border border-blue-200',   dot: 'bg-blue-500',   icon: Shield  },
  Estagiário: { badge: 'bg-amber-50 text-amber-700 border border-amber-200', dot: 'bg-amber-500',  icon: Users   },
};

const statusConfig: Record<StatusUsuario, { badge: string; dot: string }> = {
  Ativo:    { badge: 'bg-emerald-50 text-emerald-700 border border-emerald-200', dot: 'bg-emerald-500' },
...
export function CMSMobile() {
  ...
  const validateTeam = (): boolean => {
    ...
    else if (!/\S+@\S+\.\S+/.test(teamForm.email))
                                   e.email    = "E-mail inválido";
    setTeamErrors(e);
    return Object.keys(e).length === 0;
  };
  ...
  const handleTeamSave = async () => {
    ...
    const newMember: TeamMember = {
      ...
      status: 'Pendente',
      avatar: teamForm.nome.split(' ').map((n) => n[0]).slice(0, 2).join(''),
      ...
      >
        ...
        <div className="flex gap-3">
          {(selectedTab === "equipe" ? [
            ...
            { value: teamMembers.filter((m) => m.status === "Ativo").length,  label: "Ativos",       color: "text-emerald-600" },
            { value: teamMembers.filter((m) => m.nivel === "Admin").length,   label: "Admins",       color: "text-blue-600" },
          ] : [
            { value: lawyers.length,                                   label: "Advogados",   color: "text-[#1A2B3C]" },
            { value: leads.filter((l) => l.status === "novo").length,  label: "Novos Leads", color: "text-blue-600"   },
              ...
              >
                <div className="flex gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-[#1A2B3C] mb-0.5 truncate">{l.name}</h3>
                ...
                <p className="text-xs text-slate-600 mb-3 pb-3 border-b border-slate-100 leading-relaxed">{l.bio}</p>
                ...
                <div className="flex items-center gap-1.5 pt-3 border-t border-slate-100">
                  ...
                  <span className="text-xs font-semibold text-[#1A2B3C]">{l.casesActive} casos ativos</span>
                </div>
              </div>
            ))}
          </TabsContent>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-semibold text-[#1A2B3C] mb-1 truncate">{member.nome}</h3>
                      <div className="flex items-center gap-1.5 mb-1">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${nivelCfg.badge}`}>
                          <NivelIcon className="w-2.5 h-2.5" />
                          {member.nivel}
                        </span>
                      </div>
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${statusCfg.badge}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${statusCfg.dot} ${member.status === 'Ativo' ? 'animate-pulse' : ''}`} />
                        {member.status}
                      </span>
                    </div>
                    ...
                    <div className="w-full h-2 rounded-full bg-slate-200 overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          member.nivel === 'Admin' ? 'bg-[#1A2B3C]' :
                          member.nivel === 'Advogado' ? 'bg-blue-500' : 'bg-amber-500'
                        }`}
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">Preencha os dados do profissional</p>
              </div>
              <button
                onClick={closeSheet}
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  ...
                  {Object.values(permissoesPadrao[teamForm.nivel]).filter(Boolean).length > 3 && (
                    <span className="text-[10px] text-slate-400">
                      +{Object.values(permissoesPadrao[teamForm.nivel]).filter(Boolean).length - 3} mais
                    </span>
                  )}
                </div>
              </div>
