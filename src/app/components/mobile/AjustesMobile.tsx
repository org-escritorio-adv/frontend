...
import {
  ...
  LogOut, ChevronRight, Shield, Download, Trash2,
  Edit3,
} from "lucide-react";
...
const settingsSections: { title: string; items: SettingItem[] }[] = [
  {
    title: "Conta",
    items: [
      { icon: User,    label: "Editar Perfil",   description: "Nome, foto e informações",  action: "navigate" },
      { icon: Lock,    label: "Alterar Senha",   description: "Segurança da sua conta",    action: "navigate" },
  ...
  },
  {
    title: "Notificações",
    items: [
      { icon: Bell, label: "Notificações Push",  description: "Alertas de movimentações",       action: "toggle",   enabled: true  },
      { icon: Bell, label: "E-mail",             description: "Receber notificações por e-mail", action: "toggle",   enabled: true  },
    ],
  },
  {
    title: "Preferências",
    items: [
      { icon: Moon,  label: "Modo Escuro",  description: "Tema escuro para o app",  action: "toggle",   enabled: false },
  ...
  },
  {
    title: "Dados",
    items: [
      { icon: Download, label: "Exportar Dados", description: "Baixe todos os seus dados",   action: "navigate" },
      { icon: Trash2,   label: "Limpar Cache",   description: "Libere espaço do dispositivo", action: "navigate" },
    ],
  },
  {
    title: "Suporte",
    items: [
      { icon: HelpCircle, label: "Central de Ajuda",   description: "FAQ e tutoriais", action: "navigate" },
      { icon: HelpCircle, label: "Contatar Suporte",   description: "Fale conosco",    action: "navigate" },
    ],
  },
];
...
export function AjustesMobile() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* ── User Profile Card ─────────────────────────── */}
      <div className="bg-[#1A2B3C] px-4 pt-5 pb-8 rounded-b-[32px]"
              ...
              {section.items.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Text */}
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[#1A2B3C] leading-tight">{item.label}</p>
