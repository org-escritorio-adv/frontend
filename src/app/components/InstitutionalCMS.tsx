...
import {
  Edit, Mail, User, Plus, Trash2, X,
  Send, Archive, CheckCircle, Scale,
  Briefcase, Hash, AtSign, ChevronDown,
  Inbox, Clock, Phone,
} from 'lucide-react';
...
function ModalAdvogado({ isOpen, mode, initial, onClose, onSave }: ModalAdvogadoProps) {
  ...
  useEffect(() => { setForm(initial); setSucesso(false); }, [isOpen, initial]);
  ...
  const especialidades = [
    'Direito Empresarial', 'Direito Tributário', 'Direito Trabalhista',
    'Direito Civil', 'Direito Penal', 'Direito Previdenciário',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        onClick={onClose}
        className="absolute inset-0 bg-[#1A2B3C]/50 backdrop-blur-[2px]"
      />
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {mode === 'add' ? 'Preencha os dados do novo membro da equipe' : `Editando perfil de ${initial.nome ?? ''}`}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-slate-50/60">
          ...
          >
            {sucesso ? (
              ...
              <>{mode === 'add' ? <Plus className="w-4 h-4" /> : <Edit className="w-4 h-4" />} {mode === 'add' ? 'Adicionar' : 'Salvar Alterações'}</>
            )}
          </button>
        </div>
      </div>
      ...
      >
        ...
        <div className="mx-6 mb-4 p-3 rounded-xl bg-slate-50 border border-gray-100 flex-shrink-0">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="flex items-center gap-1 text-[10px] text-slate-400">
              <Clock className="w-3 h-3" />{new Date(lead.data).toLocaleDateString('pt-BR')}
            </span>
          </div>
        </div>
...
export function InstitutionalCMS() {
  ...
  const [leads, setLeads] = useState<Lead[]>([
    {
      ...
      mensagem: 'Preciso de assistência jurídica em um litígio tributário envolvendo recuperação de ICMS.',
      data: '2026-04-05',
      status: 'novo',
    },
    {
  ...
  const abrirAdicionarAdvogado = () =>
    setAdvModal({ isOpen: true, mode: 'add', initial: {} });

  const abrirEditarAdvogado = (adv: Advogado) =>
    setAdvModal({ isOpen: true, mode: 'edit', initial: { ...adv } });

  const fecharAdvModal = () =>
    setAdvModal((prev) => ({ ...prev, isOpen: false }));
  ...
  const salvarAdvogado = (dados: Partial<Advogado>) => {
    if (advModal.mode === 'add') {
      ...
      setAdvogados((prev) =>
        prev.map((a) => (a.id === dados.id ? { ...a, ...dados } : a)),
      );
    }
    fecharAdvModal();
  };

  const excluirAdvogado = (id: number) =>
    setAdvogados((prev) => prev.filter((a) => a.id !== id));

  const abrirResponder = (lead: Lead) =>
    setLeadModal({ isOpen: true, lead });

  const fecharLeadModal = () =>
    setLeadModal({ isOpen: false, lead: null });

  const arquivarLead = (id: number) =>
    setArquivados((prev) => new Set([...prev, id]));

  const desarquivarLead = (id: number) =>
    setArquivados((prev) => { const s = new Set(prev); s.delete(id); return s; });
    ...
    <>
      ...
      <div className="p-8 max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600">Gerencie perfis de advogados e consultas do site institucional</p>
        </div>
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Consultas recebidas pelo site institucional</p>
              </div>
            </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${cfg.badge}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${lead.status === 'novo' ? 'bg-emerald-500 animate-pulse' : lead.status === 'contatado' ? 'bg-blue-500' : 'bg-slate-400'}`} />
